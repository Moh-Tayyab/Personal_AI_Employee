...
"""
    ...
    def _update_dashboard(self, needs_action: list, pending: list, approved: list):
        ...
        # Count Gold Tier features
        briefings_count = len(list((self.vault_path / 'Briefings').glob('*.md'))) if (self.vault_path / 'Briefings').exists() else 0
        ...
        for action in recent_actions:
            ...
            activity_lines.append(f"- [{timestamp}] {action_type}")

        if not activity_lines:
            activity_lines.append("- No recent activity")

        dashboard = f"""# AI Employee Dashboard — Gold Tier

## System Status
- **Last Updated**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
- **Status**: {'🟢 Active' if self.running else '🔴 Stopped'}
- **Reasoning Engine**: Claude Code / Qwen Code
- **Mode**: {'🔒 DRY RUN (Safe)' if self.dry_run else '🔓 LIVE (Real Actions)'}
- **Ralph Loop**: {'🔄 Active' if self.ralph_mode else '⏸️ Inactive'}
- **Tier**: 🏆 Gold

## Queue Status
| Folder | Count | Description |
|--------|-------|-------------|
| Needs_Action | {len(needs_action)} | Items awaiting processing |
| In_Progress | {in_progress_count} | Items being processed |
| Pending_Approval | {len(pending)} | Awaiting human approval |
| Approved | {len(approved)} | Ready for execution |
...
| CEO Briefing | ✅ {briefings_count} briefing(s) |
| Approval Workflow | ✅ Active |
| Ralph Wiggum Loop | {'✅ Active' if self.ralph_mode else '⏸️ Ready'} |
...
## Quick Actions
- [ ] Check Needs_Action folder ({len(needs_action)} items)
- [ ] Review Pending_Approval ({len(pending)} items)
- [ ] Execute Approved items ({len(approved)} items)
...
*Next CEO Briefing: {(datetime.now() + timedelta(days=7)).strftime('%Y-%m-%d')}*
"""
